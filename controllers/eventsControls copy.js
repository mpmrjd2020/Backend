...
router.delete('/event/:eventDeleteID', (req, res) => {
    eventSchema.findOne(req.params.eventDeleteID).then
    (e => {
        e.items.map(itemToDelete => {
            itemSchema.findByIdAndDelete(itemToDelete).then(iDelete => {
                res.json(iDelete)
            })
        e.seller.map(sellerToDelete => {
            itemSchema.findByIdAndDelete(sellerToDelete).then(sDelete => {
                res.json(sDelete)
            })
        })
    })
